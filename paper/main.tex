...
%Because they need to enumerate an exponential number of combinations,
Approaches that attempt to consider all possible edit states as individual target sequences must consider all combinations of edits, which increases complexity and can lead to errors \citep{huang2013short}, \citep{mun2021leviosam}.
Additionally, when assessing the success rate of genomic edits, a linear alignment method may introduce reference bias, particularly when multiple nearby edits and edit state mixtures are present.

Alternatively, a genome graph approach can provide a more complete and accurate perspective of sequence relationships, while avoiding reference bias and capturing all allele state mixtures present within the targeted edits, even in the case of overlapped edits \citep{eggertsson2017graphtyper,Martiniano_2020}.
Compared to linear alignment methods, genome graphs can provide a more accurate and comprehensive view \citep{garrison2018variation} of the relationships between sequences \citep{paten2017genome} because reads can be mapped to a graph that captures a population of sequences and variation between them, instead of to a single reference.

Given these advantages over linear methods, we suggest a pipeline based on genome graph alignment tools would be better suited for quantifying edit efficacy in multiple populations of edited cells.
Here, we present \textit{FantasticLamp}, a simple pipeline that uses genome graph based sequence analysis to quantify edit efficacy in experiments where multiple edits were performed separately on the same population.

Our approach maps genomic edits onto structures that we can represent in variation graphs (Figure \ref{fig:images}A-C).
We then align reads drawn from a population of edited cells to this graph, and quantify edit efficiency by comparing our observations for edited and unedited alleles states.
\end{comment}
