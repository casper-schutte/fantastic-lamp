...
def make_coverage_table(path_ids):
    ...
    for path in path_ids:
        if path[1] != 0 and path[3] != 0:
            if tally_reads_in_path(path[0]) == 0:
                frac_cov = 0
            elif tally_reads_in_path(path[2]) == 0 and tally_reads_in_path(path[0]) != 0:
                frac_cov = 1
            elif tally_reads_in_path(path[2]) == 0 and tally_reads_in_path(path[0]) == 0:
                frac_cov = 0
            else:
                frac_cov = (tally_reads_in_path(path[0]) / path[1]) / (
                        (tally_reads_in_path(path[0]) / path[1]) + (tally_reads_in_path(path[2]) / path[3]))
            coverage_list.append(
                [path[0], frac_cov,
                 tally_reads_in_path(path[0]) / path[1],
...
def write_to_tsv(coverage_list):
    ...
    with open(args.out_path, "wt") as tsv_file:
        ...
        tsv_writer.writerow(["Homology arm", "Fractional Homology arm coverage", "Homology arm coverage", "Reference "
                                                                                                          "coverage",
                             "Homology arm edges",
